    ...
    public void updateCollision(BasicBlock[][] mapArray, int x, int y, boolean needCollision) {
        if (needCollision) {
            if (x + 1 < GameSettings.MAP_WIDTH)
                if (mapArray[x + 1][y] != null) mapArray[x + 1][y].setHasCollision(true);
            if (x - 1 >= 0)
                if (mapArray[x - 1][y] != null) mapArray[x - 1][y].setHasCollision(true);
            if (y + 1 < GameSettings.MAP_HEIGHT)
                if (mapArray[x][y + 1] != null) mapArray[x][y + 1].setHasCollision(true);
            if (y - 1 >= 0)
                if (mapArray[x][y - 1] != null) mapArray[x][y - 1].setHasCollision(true);
        }
        else {
            for (int i = 0; i < 4; i++) {
                ...
                if (blockX >= 0 && blockX < GameSettings.MAP_WIDTH && blockY >= 0 && blockY < GameSettings.MAP_HEIGHT)
                    if (!hasBlockNearBy(mapArray, blockX, blockY) && mapArray[blockX][blockY] != null && blockY != GameSettings.MAP_HEIGHT - 1)
